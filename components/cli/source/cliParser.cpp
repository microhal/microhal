...
CliParser::ParserStatus CliParser::parseNumerical(std::list<char *> &words, std::pair<float, char *> &val, float defalt, float min, float max,
                                                  int base) {
    ParserStatus fin = ParserStatus::NoMnemonic;
    if (!words.size()) return fin;
...
template <typename T>
bool CliParser::argumentCheck(std::list<char *> &words, std::pair<T, char *> &val) {
