...
def runEclipse(project, target, defines):
	...
	return os.popen("arm-none-eabi-size " + project + '/jenkins/stm32f4xx_stat_i2c.elf').read()
...
def createDefinesList(configuration, numberOfserialPorts):
	...
	for i in range(1,numberOfserialPorts+1):
		...
		for j in range(1, i+1): 
			defines.append('MICROHAL_USE_I2C' + str(j) + '_' + configuration)
...
def buildConfiguration(config, numberOfserialPorts):
	...
	for defines in definesList:
		sizeOutput.append(runEclipse(projectDir, 'stm32f4xx_stat_i2c/jenkins', defines))
...
saveFile('i2c_polling_text_size.txt', pollingSize, 'text')
saveFile('i2c_polling_data_size.txt', pollingSize, 'data')
saveFile('i2c_polling_bss_size.txt', pollingSize, 'bss')

interruptSize = buildConfiguration('INTERRUPT', 3)
saveFile('i2c_interrupt_text_size.txt', interruptSize, 'text')
saveFile('i2c_interrupt_data_size.txt', interruptSize, 'data')
saveFile('i2c_interrupt_bss_size.txt', interruptSize, 'bss')

dmaSize = buildConfiguration('DMA', 3)
saveFile('i2c_dma_text_size.txt', dmaSize, 'text')
saveFile('i2c_dma_data_size.txt', dmaSize, 'data')
saveFile('i2c_dma_bss_size.txt', dmaSize, 'bss')
