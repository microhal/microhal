...
def program(fullPathToBinary, board):  
    ...
    parameters = '-d -f board/' + boardType + ' -c "hla_serial ' + serial + '" -c init -c targets -c "reset halt" -c "sleep 100" -c "flash write_image erase ' + fullPathToBinary + '" -c "verify_image ' + fullPathToBinary +'" -c "reset run" -c shutdown'	
