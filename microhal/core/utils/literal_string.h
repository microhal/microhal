...
#include <cstdint>
#include <utility>
#include "diagnostic/compileTime_itoa.h"
...
struct string {
    ...
    constexpr string(char const (&str)[Size + 1]) noexcept : string(str, std::make_index_sequence<Size>()) {}
    ...
    constexpr string<Size + 1> push_back(char c) const noexcept { return push_back(c, std::make_index_sequence<Size>()); }

    constexpr string<Size + 1> push_front(char c) const noexcept { return push_front(c, std::make_index_sequence<Size>()); }
 ...
 private:
    ...
    constexpr string(char const (&str)[I], std::index_sequence<Indices...>) noexcept : arr{str[Indices]..., '\0'} {}
