...
class CRC16 {
 public:
    ...
    uint16_t crc16(uint16_t *data, uint16_t *last) { return (data == last) ? 0xFFFF : *data * crc16(data + 1, last); }
    ...
    static uint16_t ByteCRC16(int value, int crcin) { return static_cast<uint16_t>((crcin << 8) ^ ccittTable[((crcin >> 8) ^ (value)) & 255]); }
