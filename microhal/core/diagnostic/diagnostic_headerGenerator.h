...
class LogLevelHeader : public LogLevelHeader_base {
 public:
    explicit constexpr LogLevelHeader(const char *levelFileLineInfo, const char *code) noexcept : LogLevelHeader_base(levelFileLineInfo, code) {}
...
struct LogPositionContainer {
    ...
    constexpr LogPositionContainer(std::index_sequence<FilenameIndices...> s, std::index_sequence<lineIndices...> a, const char *filename,
                                   const char *line)
...
struct HeaderContainer {
    constexpr HeaderContainer(LogPositionContainer<N> filename, const char *errorCode)
        : filename(filename), header(LogLevelHeader<level>(this->filename.str, errorCode)) {}
...
constexpr auto generate(const T file) {
    ...
    constexpr auto charToUint = [](auto const charStr) {
        std::array<uint8_t, sizeof(charStr)> uintStr{};
        for (size_t i = 0; i < uintStr.size(); i++) {
            uintStr[i] = charStr[i];
        }
        return uintStr;
    };
    constexpr const auto uintStr = charToUint(logPosition.str);
    // let calculate hash of logPosition text, this will produce unique log code
    constexpr auto crc = CRC32<>::calculate(uintStr.data(), uintStr.size());
    constexpr const char *errorCode = ErrorCode<file.logLevel(), crc>::c_str();
