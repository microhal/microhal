...
#ifndef SRC_MICROHAL_PORTS_STMCOMMON_CLOCKMANAGER_SDIOCLOCK_H_
#define SRC_MICROHAL_PORTS_STMCOMMON_CLOCKMANAGER_SDIOCLOCK_H_
...
#if defined(_MICROHAL_SDIO_BASE_ADDRESS)
#if defined(_MICROHAL_CLOCKMANAGER_HAS_POWERMODE) && _MICROHAL_CLOCKMANAGER_HAS_POWERMODE == 1

static void enableSDIO(PowerMode mode) {
    auto apb2enr = registers::rcc->apb2enr.volatileLoad();
    auto apb2lpenr = registers::rcc->apb2lpenr.volatileLoad();
    if (isEnabled(mode, PowerMode::Normal)) apb2enr.SDIOEN.set();
    if (isEnabled(mode, PowerMode::Sleep)) apb2lpenr.SDIOLPEN.set();
    registers::rcc->apb2lpenr.volatileStore(apb2lpenr);
    registers::rcc->apb2enr.volatileStore(apb2enr);
}
static void disableSDIO(PowerMode mode) {
    auto apb2enr = registers::rcc->apb2enr.volatileLoad();
    auto apb2lpenr = registers::rcc->apb2lpenr.volatileLoad();
    if (isEnabled(mode, PowerMode::Normal)) apb2enr.SDIOEN.clear();
    if (isEnabled(mode, PowerMode::Sleep)) apb2lpenr.SDIOLPEN.clear();
    registers::rcc->apb2lpenr.volatileStore(apb2lpenr);
    registers::rcc->apb2enr.volatileStore(apb2enr);
}
...
#endif /* SRC_MICROHAL_PORTS_STMCOMMON_CLOCKMANAGER_SDIOCLOCK_H_ */
