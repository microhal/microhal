...
class IOManager {
 public:
    ...
    static void routeTimer(stm32f4xx::GPIO::PullType pull = stm32f4xx::GPIO::NoPull, stm32f4xx::GPIO::OutputType type = stm32f4xx::GPIO::PushPull) {
        stm32f4xx::GPIO::setAlternateFunction(stm32f4xx::GPIO::PortA, 2, stm32f4xx::GPIO::Timer_1_2, stm32f4xx::GPIO::NoPull,
    ...
    static void routeSerial(stm32f4xx::GPIO::PullType pull = stm32f4xx::GPIO::NoPull, stm32f4xx::GPIO::OutputType type = stm32f4xx::GPIO::PushPull) {
        // assert for Serial1
        static_assert((serial != 1 || serialType != Txd || ((port == GPIO::PortA && pinNr == 9) || (port == GPIO::PortB && pinNr == 6))),
                      "Serial1 Txd can be connected only to: PortA.9 or PortB.6.");
        static_assert((serial != 1 || serialType != Rxd || ((port == GPIO::PortA && pinNr == 10) || (port == GPIO::PortB && pinNr == 7))),
                      "Serial1 Rxd can be connected only to: PortA.10 or PortB.7.");
// assert for Serial2
#if !defined(STM32F410Tx) && !defined(STM32F410Cx) && !defined(STM32F410Rx)
        static_assert((serial != 2 || serialType != Txd || ((port == GPIO::PortA && pinNr == 2) || (port == GPIO::PortD && pinNr == 5))),
                      "Serial2 Txd can be connected only to: PortA.2 or PortD.5.");
        static_assert((serial != 2 || serialType != Rxd || ((port == GPIO::PortA && pinNr == 3) || (port == GPIO::PortD && pinNr == 6))),
...
// assert for Serial3
        ...
        static_assert((serial != 3 || serialType != Txd ||
                       ((port == GPIO::PortB && pinNr == 10) || (port == GPIO::PortC && pinNr == 10) || (port == GPIO::PortD && pinNr == 8))),
                      "Serial3 Txd can be connected only to: PortB.10 or PortC.10 or PortD.8.");
        static_assert((serial != 3 || serialType != Rxd ||
                       ((port == GPIO::PortB && pinNr == 11) || (port == GPIO::PortC && pinNr == 11) || (port == GPIO::PortD && pinNr == 9))),
                      "Serial3 Rxd can be connected only to: PortB.11 or PortC.11 or PortD.9.");
#else
        static_assert((serial != 3 || serialType != Txd || ((port == GPIO::PortB && pinNr == 10) || (port == GPIO::PortC && pinNr == 10))),
                      "Serial3 Txd can be connected only to: PortB.10 or PortC.10.");
        static_assert((serial != 3 || serialType != Rxd || ((port == GPIO::PortB && pinNr == 11) || (port == GPIO::PortC && pinNr == 11))),
                      "Serial3 Rxd can be connected only to: PortB.11 or PortC.11.");
#endif
        // assert for Serial4
        static_assert((serial != 4 || serialType != Txd || (port == GPIO::PortA && pinNr == 0) || (port == GPIO::PortC && pinNr == 10)),
                      "Serial4 Txd can be connected to: PortA.0 or PortC.10.");
        static_assert((serial != 4 || serialType != Rxd || (port == GPIO::PortA && pinNr == 1) || (port == GPIO::PortC && pinNr == 11)),
                      "Serial4 Rxd can be connected to: PortA.1 or PortC.11.");
        // assert for Serial5
        static_assert((serial != 5 || serialType != Txd || ((port == GPIO::PortC && pinNr == 12) || (port == GPIO::PortC && pinNr == 3))),
                      "Serial5 Txd can be connected only to: PortC.12 or PortC.3.");
#if !defined(STM32F410Tx) && !defined(STM32F410Cx) && !defined(STM32F410Rx)
        static_assert((serial != 5 || serialType != Rxd || ((port == GPIO::PortD && pinNr == 2) || (port == GPIO::PortA && pinNr == 3))),
                      "Serial5 Rxd can be connected only to: PortD.2, PortA.3.");
#else
        static_assert((serial != 5 || serialType != Rxd || ((port == GPIO::PortA && pinNr == 2))), "Serial5 Rxd can be connected only to: PortA.3.");
#endif
        // assert for Serial6
...
// assert for Serial8
    ...
    static void routeSPI(stm32f4xx::GPIO::PullType pull = stm32f4xx::GPIO::NoPull, stm32f4xx::GPIO::OutputType type = stm32f4xx::GPIO::PushPull) {
        ...
        // assert for SPI1
        static_assert((spiNumber != 1 || spiType != SCK || ((port == GPIO::PortA && pinNr == 5) || (port == GPIO::PortB && pinNr == 3))),
                      "SPI1 SCK can be connected only to: PortA.5 or PortB.3.");
        static_assert((spiNumber != 1 || spiType != MISO || ((port == GPIO::PortA && pinNr == 6) || (port == GPIO::PortB && pinNr == 4))),
                      "SPI1 MISO can be connected only to: PortA.6 or PortB.4.");
        static_assert((spiNumber != 1 || spiType != MOSI || ((port == GPIO::PortA && pinNr == 7) || (port == GPIO::PortB && pinNr == 5))),
                      "SPI1 MOSI can be connected only to: PortA.7 or PortB.5.");
        // assert for SPI2
        static_assert((spiNumber != 2 || spiType != SCK || ((port == GPIO::PortB && pinNr == 10) || (port == GPIO::PortB && pinNr == 13))),
                      "SPI2 SCK can be connected only to: PortB.10 or PortB.13.");
        static_assert((spiNumber != 2 || spiType != MISO || ((port == GPIO::PortB && pinNr == 14) || (port == GPIO::PortC && pinNr == 2))),
                      "SPI2 MISO can be connected only to: PortB.14 or PortC.2.");
        static_assert((spiNumber != 2 || spiType != MOSI || ((port == GPIO::PortB && pinNr == 15) || (port == GPIO::PortC && pinNr == 3))),
                      "SPI2 MOSI can be connected only to: PortB.15 or PortC.3.");
        // assert for SPI3
        static_assert((spiNumber != 3 || spiType != SCK || ((port == GPIO::PortB && pinNr == 3) || (port == GPIO::PortC && pinNr == 10))),
                      "SPI3 SCK can be connected only to: PortB.3 or PortC.10.");
        static_assert((spiNumber != 3 || spiType != MISO || ((port == GPIO::PortB && pinNr == 4) || (port == GPIO::PortC && pinNr == 11))),
                      "SPI3 MISO can be connected only to: PortB.4 or PortC.11.");
        static_assert((spiNumber != 3 || spiType != MOSI || ((port == GPIO::PortB && pinNr == 5) || (port == GPIO::PortC && pinNr == 12))),
    ...
    static void routeI2C(stm32f4xx::GPIO::PullType pull = stm32f4xx::GPIO::NoPull, stm32f4xx::GPIO::OutputType type = stm32f4xx::GPIO::OpenDrain) {
        ...
        // assert for I2C1
        static_assert((i2cNumber != 1 || i2cType != SDA || ((port == GPIO::PortB && pinNr == 7) || (port == GPIO::PortB && pinNr == 9))),
                      "I2C1 SDA can be connected only to: PortB.7 or PortB.9.");
        static_assert((i2cNumber != 1 || i2cType != SCL || ((port == GPIO::PortB && pinNr == 6) || (port == GPIO::PortB && pinNr == 8))),
