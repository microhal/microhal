...
#endif
I2C_dma I2C_dma::i2c1(*I2C1, DMA::dma1->stream[MICROHAL_I2C1_DMA_RX_STREAM],
                      DMA::dma1->stream[MICROHAL_I2C1_DMA_TX_STREAM]);
...
#endif
I2C_dma I2C_dma::i2c2(*I2C2, DMA::dma1->stream[MICROHAL_I2C2_DMA_RX_STREAM],
                      DMA::dma1->stream[7]);
 ...
 * @return
 */
I2C::Error I2C_dma::write(DeviceAddress deviceAddress, uint8_t data) {
  transfer.mode = Transmit;
  transfer.deviceAddress = deviceAddress;
  transfer.txLength = sizeof(uint8_t);

  txStream.setNumberOfItemsToTransfer(sizeof(uint8_t));
  txStream.setMemoryBank0(&data);
  txStream.enable();

  errorSemaphore = UnknownError;

  while (i2c.SR1 & I2C_CR1_STOP) {
  }
...
I2C::Error I2C_dma::write(DeviceAddress deviceAddress,
...
I2C::Error I2C_dma::read(uint8_t deviceAddress, uint8_t &data) {
  ...
  transfer.deviceAddress = deviceAddress;
  transfer.rxLength = sizeof(uint8_t);

  rxStream.setNumberOfItemsToTransfer(sizeof(uint8_t));
 ...
 * @return
 */
I2C::Error I2C_dma::read(DeviceAddress deviceAddress,
						 uint8_t * read_data,
...
void I2C_dma::init(void) {
  ...
  // rx
  rxStream.deinit();
  rxStream.init(getChannalNumber(i2c), DMA::Stream::MemoryBurst::SingleTransfer,
...
//***********************************************************************************************//
#ifdef MICROHAL_USE_I2C1_DMA
void I2C2_EV_IRQHandler(void) { I2C_dma::IRQFunction(I2C_dma::i2c1, I2C1); }
#endif
#ifdef MICROHAL_USE_I2C2_DMA
