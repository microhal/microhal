...
public:
	...
	void peripheralAddress(volatile void *addr) {
		channel.CPAR = reinterpret_cast<uint32_t>(addr);
	}
	void memoryAddress(void *memoryAddr) {
		channel.CMAR = reinterpret_cast<uint32_t>(memoryAddr);
	}

	void* memoryAddress() {
		return reinterpret_cast<void*>(channel.CMAR);
	}

	void numberOfItemsToTransfer(size_t len) {
		channel.CNDTR = len;
	}
	size_t numberOfItemsInTransfer() const {
...
public:
	...
	void enableInterrupt(const Channel& stream, uint32_t priority) noexcept {
		const IRQn_Type irq[7] = { DMA1_Channel1_IRQn, DMA1_Channel2_IRQn, DMA1_Channel3_IRQn, DMA1_Channel4_IRQn, DMA1_Channel5_IRQn, DMA1_Channel6_IRQn, DMA1_Channel7_IRQn};
 ...
 private:
	uint32_t calculateStreamNumber(const Channel &stream) const noexcept {
