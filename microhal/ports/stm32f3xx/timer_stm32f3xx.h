 ...
 public:
    ...
    enum class ADCSynchronizationEvent {
        ...
        Enable, /* the Counter Enable signal CNT_EN is used as trigger output (TRGO2). It is useful to start several timers at the same time or to
                 * control a window in which a slave timer is enabled. The Counter Enable signal is generated by a logic OR between the CEN control
                 * bit and the trigger input when configured in Gated mode. When the Counter Enable signal is controlled by the trigger input, there
                 * is a delay on TRGO2, except if the Master/Slave mode is selected (see the MSM bit description in TIMx_SMCR register).
                 **/
        Update, /* the update event is selected as trigger output (TRGO2). For instance, a master timer can then be used as a prescaler for a slave
                 * timer.
                 */
    ...
    uint32_t getInternalClockFrequency() { return ClockManager::TimerFrequency(*(TIM_TypeDef *)&timer); }
