...
bool M24C16::write(size_t address, const gsl::not_null<const void *> data, size_t data_size) {
	...
	if (pageWrite(address, data, size) != Error::NoError) return false;
	dataToWrite -= size;
	...
	if(dataToWrite != 0) {
		...
		for (size_t i = 0; i< pagesToWrite; i++) {
			...
			if (pageWrite(address, ptr, pageSize)!= Error::NoError) return false;
			address += pageSize;
			ptr += pageSize;
		}
		// wait for finish previous write operation
		...
		if (pageWrite(address, ptr, lastPageSize)!= Error::NoError) return false;
	}
	return true;
}
