...
class BMP {
 ...
 public:
	...
	struct Picture {
		...
		Picture(size_t width, size_t height) : width(width), height(height) {
		...
		Pixel * getLinePtr(size_t y) {
			return &data[y * width];
		}

		const Pixel * getLinePtr(size_t y) const {
			return &data[y * width];
		}
	};
	...
	bool save(const char* filename, const Picture &picture) {
		...
		if (FILE *f = fopen(filename, "wb")) {
			...
			for(size_t i = 0; i < picture.height; i++)			{
				fwrite(picture.getLinePtr(i), sizeof(Pixel), picture.width, f);
				fwrite(padding, 1, picture.calculatePadding(picture.width),f);
			}
			fclose(f);
			return true;
		}
...
void saveColoredImage(const Picture &picture, uint32_t pictureNumber) {
	...
	for (size_t y = 0; y < picture.hight(); ++y) {
        for (size_t x = 0; x < picture.width(); ++x) {
          ...
          //  baseValue = microhal::byteswap(baseValue);
            int scaledValue = 256 * (baseValue - *minmax.first) / diff; // map value to interval [0, 256), and set the pixel to its color value above
            image.pixel(x, y) = BMP::Pixel {colormap[3*scaledValue + 2], colormap[3*scaledValue+1], colormap[3*scaledValue]};
        }
	}

	static char pictureName[100];
	BMP pic;
	sprintf(pictureName, "pictures/img%d.bmp", pictureNumber);
	pic.save(pictureName, image);
}

Picture gpicture;
uint32_t num;
volatile uint32_t newPicture = 0;
...
std::queue<PictureData> pictureQueue;

void proceedPacket(HostCommPacket &packet) {
	if (packet.getType() == PicturePacket::PacketType) {
	//	diagChannel << lock << Informational << "Picture received" << unlock;
		PicturePacket &picturePacket = static_cast<PicturePacket &>(packet);
		Picture &picture = picturePacket.payload();

		//memcpy(gpicture.data, picture.data, sizeof(picture.data));
		//num = picturePacket.payloadPtr()->pictureNumber;
		pictureQueue.emplace(PictureData{picturePacket.payloadPtr()->pictureNumber, picture});
		//saveColoredImage(picture,  picturePacket.payloadPtr()->pictureNumber);
	}
}
...
int main(int argc, char * argv[]) {
	...
	const char *defaultPortname = "/dev/ttyUSB1";
	const char *portname = defaultPortname;
	if (argc == 2) {
		portname = argv[1];
	}
	microhal::linux::SerialPort cameraPort(portname);
	if (cameraPort.open(SerialPort::ReadWrite) == false) {
		diagChannel << lock << MICROHAL_WARNING << " Unable to open camera port." << unlock;
		return -1;
	}
	...
	cameraPort.clear(SerialPort::AllDirections);

	static HostComm hostComm(cameraPort, debugPort);
	...
	while(1) {
		if (pictureQueue.size()) {
			newPicture--;
			saveColoredImage(pictureQueue.front().picture,  pictureQueue.front().number);
			pictureQueue.pop();
		}
