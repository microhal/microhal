...
class BMP {
 ...
 public:
    ...
    struct Picture {
        ...
        Picture(size_t width, size_t height) : width(width), height(height) { data = static_cast<Pixel *>(malloc(width * height * sizeof(Pixel))); }
        ~Picture() { free(data); }
...
void createBitmap(graphics::Color *destination, const Picture &picture) {
    const uint8_t colormap[] = {
        255, 255, 255, 253, 253, 253, 251, 251, 251, 249, 249, 249, 247, 247, 247, 245, 245, 245, 243, 243, 243, 241, 241, 241, 239, 239, 239, 237,
        237, 237, 235, 235, 235, 233, 233, 233, 231, 231, 231, 229, 229, 229, 227, 227, 227, 225, 225, 225, 223, 223, 223, 221, 221, 221, 219, 219,
        219, 217, 217, 217, 215, 215, 215, 213, 213, 213, 211, 211, 211, 209, 209, 209, 207, 207, 207, 205, 205, 205, 203, 203, 203, 201, 201, 201,
    ...
    for (size_t h = 0; h < picture.hight(); ++h) {
        for (size_t w = 0; w < picture.width(); ++w) {
            auto baseValue = picture.data[picture.width() * h + w];      // take input value in [0, 65536)
            int scaledValue = 256 * (baseValue - *minmax.first) / diff;  // map value to interval [0, 256), and set the pixel to its color value above
            destination[w + (h * picture.width())] =
                graphics::Color{colormap[3 * scaledValue + 2], colormap[3 * scaledValue + 1], colormap[3 * scaledValue]};
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    const char *defaultPortname = "/dev/ttyUSB0";
    ...
    GenericDisplay display(80, 60, 2);
    display.init();
    ...
    while (1) {
        if (pictureQueue.size()) {
            ...
            graphics::Color bitmap[60 * 80];
            createBitmap(bitmap, pictureQueue.front().picture);
            display.drawBitmap({0, 0}, 80, 60, bitmap);
            pictureQueue.pop();
        }
